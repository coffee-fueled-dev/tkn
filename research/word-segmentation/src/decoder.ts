 ...
 */
export const asPlaintext = (buffer: Uint8Array): string => {
  ...
  if (buffer.length === 1) {
    ...
    if (byte < 32 || byte === 127) {
      return formatControlCharacter(byte);
    }
  ...
  try {
    ...
    if (
      ...
      return formatControlSequence(buffer);
    }

    return decoded;
  } catch (error) {
    // Invalid UTF-8 sequence - show as hex
    return formatHexSequence(buffer);
  }
};
 ...
 */
const formatControlCharacter = (byte: number): string => {
 ...
 */
const formatControlSequence = (buffer: Uint8Array): string => {
 ...
 */
const formatHexSequence = (buffer: Uint8Array): string => {
